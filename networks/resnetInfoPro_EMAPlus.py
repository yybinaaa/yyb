...
from .configAuxPlus import InfoPro_Aux, InfoPro_balanced_memory_Aux
...
class InfoProResNet(nn.Module):
    ...
    def __init__(self, block, layers, arch, local_module_num, batch_size, image_size=32,
        ...
        self.Aug_Net = self._make_Aux_Net()
        self.EMA_Net = self._make_Aux_Net()
        ...
        if self.training:
            ...
            if self.local_module_num == 1:
                ...
                for i in range(len(self.Encoder_Net) - 1):
                    ...
                    ratio = local_index / (self.local_module_num - 2) if self.local_module_num > 2 else 0
                    ixx_r = ixx_1 * (1 - ratio) + ixx_2 * ratio
                    ixy_r = ixy_1 * (1 - ratio) + ixy_2 * ratio
                    loss_ixx = eval('self.decoder_' + str(local_index) + '_' + str(layer_index))(y,self._image_restore(img))
                    loss_ixy = eval('self.aux_classifier_' + str(local_index) + '_' + str(layer_index))(y,target)
