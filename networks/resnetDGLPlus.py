...
class InfoProResNet(nn.Module):
    ...
    def __init__(self, block, layers, arch, local_module_num, batch_size, image_size=32,
        ...
        for item in self.infopro_config[:-1]:
            ...
            temp.append(copy.deepcopy(eval('self.layer' + str(mo))[la]))
            # 将第一个强化层迭代成第二个强化层并加入temp
            if mo == 3 and la == len(self.layer1) - 1:
                mo2,la2 = mo,la
            elif la == len(self.layer1) - 1:
                mo2,la2 = mo + 1,0
                temp.append(copy.deepcopy(eval('self.layer' + str(mo2))[la2]))
            else:
                mo2,la2 = mo,la + 1
                temp.append(copy.deepcopy(eval('self.layer' + str(mo2))[la2]))


            self.Aug_Net.append(nn.Sequential(*copy.deepcopy(temp)))
            self.EMA_Net.append(nn.Sequential(*copy.deepcopy(temp)))
            ...
            if self.local_module_num == 1:
                ...
                for i in range(len(self.Encoder_Net) - 1):
                    if i == 0:
                        ...
                        if lo == 3 and la == len(self.layer1) - 1:
                            lo2,la2 = lo,la
                        elif la == len(self.layer1) - 1:
                            lo2, la2 = lo + 1, 0
                        else:
                        ...
                        loss = eval('self.aux_classifier_' + str(lo2) + '_' + str(la2))(y,target)
                        loss.backward()
                        ...
                        x = x.detach()
                        cur_layer1 = eval('self.layer' + str(lo))[la]
                        cur_layer2 = eval('self.layer' + str(lo2))[la2]
                        cur_layer = nn.Sequential(*nn.ModuleList([cur_layer1,cur_layer2]))
                ...
                for paramEncoder, paramEMA in zip(cur_layer.parameters(), self.EMA_Net[-1].parameters()):
                    paramEMA.data = paramEMA.data * self.momentum + paramEncoder.data * (1 - self.momentum)
            return logits, loss
