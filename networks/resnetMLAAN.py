...
from .auxMLAAN import AuxClassifier
...
class InfoProResNet(nn.Module):
    ...
    def __init__(self, block, layers, arch, local_module_num, batch_size, image_size=32,
        ...
        # for j in range(55):#一个硬编码的上限，用来一次性创建足够多的辅助分类器属性，以覆盖所有可能的本地模块索引。
        ...
        # 重新设计辅助分类器的初始化
        # 为每个辅助网络创建对应的分类器，通道数基于辅助网络的实际输出
        for i in range(self.local_module_num - 1):  # 遍历每个辅助网络
            # 获取当前辅助网络的最后一层配置
            last_aux_layer = self.aux_config[i][-1]  # 获取最后一层的配置 [layer_group, layer_index]
            aux_output_channels = self.widelist[last_aux_layer[0]]  # 根据layer_group确定输出通道数
            
            # 为这个辅助网络创建分类器
            for module_index in range(1, 4):
                for layer_index in range(len(self.layer1)):
                    exec('self.aux_classifier_' + str(module_index) + '_' + str(layer_index) + '_' + str(i) +
                         f'= AuxClassifier({aux_output_channels}, class_num=class_num, '
                         'widen=aux_net_widen, feature_dim=aux_net_feature_dim)')
        
        # 为剩余的分类器索引创建默认分类器（使用64通道）
        for j in range(self.local_module_num - 1, 55):
            for module_index in range(1, 4):
                for layer_index in range(len(self.layer1)):
                    exec('self.aux_classifier_' + str(module_index) + '_' + str(layer_index) + '_' + str(j) +
                         f'= AuxClassifier(64, class_num=class_num, '
                         'widen=aux_net_widen, feature_dim=aux_net_feature_dim)')
            ...
            if self.local_module_num == 1:
                ...
                for i in range(len(self.Encoder_Net) - 1):
                    if i == 0:
                        ...
                        # # 添加调试输出
                        # print(f"Aux network {i}: z.shape = {z.shape}")
                        # print(f"infopro_config[{i}] = {self.infopro_config[i]}")
                        # print(f"aux_config[{i}] = {self.aux_config[i]}")
                        
                        # local_x, layer_x = self.infopro_config[i]
                        # print(f"Using classifier: aux_classifier_{local_x}_{layer_x}_{i}")
