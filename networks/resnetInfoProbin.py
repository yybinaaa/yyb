...
class InfoProResNet(nn.Module):
        ...
        for j in range(55):
            for module_index in range(1,4):
                for layer_index in range(len(self.layer1)):
                    ...
                    exec('self.y_decoder_' + str(module_index) + '_' + str(layer_index) + '_' + str(j) +
        ...
        for module_index in range(1,4):
            for layer_index in range(len(self.layer1)):
                ...
                exec('self.x_classifier_' + str(module_index) + '_' + str(layer_index) +
                     ...
                     'widen=aux_net_widen, feature_dim=aux_net_feature_dim)')
                exec('self.x_decoder_' + str(module_index) + '_' + str(layer_index) +
    ...
    #EMA_Net：这是辅助网络的副本，通常用于训练过程中进行参数平滑。
    # 它和 Aux_Net 的结构相同，但参数值是基于历史模型的指数移动平均。
            ...
            if self.local_module_num == 1:
                ...
                for i in range(len(self.Encoder_Net) - 1):
                    ...
                    if i == 0:
                        ...
                        loss_y1 = eval('self.y_decoder_' + str(local_x) + '_' + str(layer_x) + '_' + str(i))(z, self._image_restore(img))
                        loss_y2 = eval('self.y_classifier_' + str(local_x) + '_' + str(layer_x) + '_' + str(i))(z, target)
